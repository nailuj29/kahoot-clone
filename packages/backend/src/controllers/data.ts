...
export const quizzes_post: Handler = async (req, res) => {
  const { title, questions, private: isPrivate = false } = req.body as { [key: string]: unknown };
  let quiz: Quiz;
  if (typeof title == "string" && typeof isPrivate == "boolean") {
    try {
      ...
      res.status(500).send({ status: 500, message: "An unknown error occurred while creating your quiz "});
      return;
    }
  }
  ...
  for (const question of questionsList) {
    try {
      if (!(question.answers && question.correctAnswers && question.text)) {
        res.status(400).json({ status: 400, message: "You must provide a title, name, and answers for your question"});
        return;
      }
